...
class AbstractTrainer(metaclass=ABCMeta):
    def __init__(self, args, model, train_loader, val_loader, test_loader, export_root):
        ...
        self.test_loader = test_loader if test_loader else None  # set test_loader to None if it is empty
    ...
    def finetune(self):
        accum_iter = 0
        self.args.online_similarity_model.update_embedding_matrix(self.model.bert_variable.embedding.token)
        ...
        self.writer.close()
        torch.save(self.model.state_dict, os.path.join(self.export_root, 'models', 'best_acc_model.pth'))
    ...
    def finetune_one_epoch(self, epoch, accum_iter):
        self.model.train()
        average_meter_set = AverageMeterSet()
        tqdm_dataloader = tqdm(self.finetune_loader)
        cl_loader = iter(self.cl_loader)
        for batch_idx, batch in enumerate(tqdm_dataloader):

            batch_size = batch[0].size(0)
            batch_ = [x.to(self.device) for x in batch]
            l = next(cl_loader)
            cl = [x.to(self.device) for x in l]
            real_batch = [batch_[0],cl[0],cl[1],batch_[1],batch_[2]]
